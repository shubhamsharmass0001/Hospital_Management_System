...
// Allow access if either a patient (`pid`) or an authenticated user (`username`) exists.
if (!isset($_SESSION['pid']) && !isset($_SESSION['username'])) {
  echo "<script>alert('Please login to book an appointment'); window.location.href='index1.php';</script>";
  exit;
}

  $pid = isset($_SESSION['pid']) ? $_SESSION['pid'] : null;
    ...
    if((date("Y-m-d",$appdate1)==$cur_date and date("H:i:s",$apptime1)>$cur_time) or date("Y-m-d",$appdate1)>$cur_date) {
      $check_query = mysqli_query($con,"select apptime from appointmenttb where doctor='$doctor' and appdate='$appdate' and apptime='$apptime'");
        ...
        if(mysqli_num_rows($check_query)==0){
          // If no patient session, but guest fields provided, create a new patient record on-the-fly
          if(empty($pid) || !is_numeric($pid)){
            if(isset($_POST['guest_fname']) && isset($_POST['guest_email'])){
              $g_fname = trim($_POST['guest_fname']);
              $g_lname = trim($_POST['guest_lname']);
              $g_gender = trim($_POST['guest_gender']);
              $g_email = trim($_POST['guest_email']);
              $g_contact = trim($_POST['guest_contact']);

              // Validate / sanitize contact: keep only digits and require 10 digits
              $g_contact_digits = preg_replace('/\D/', '', $g_contact);
              if(strlen($g_contact_digits) !== 10){
                echo "<script>alert('Contact number must be 10 digits. Please correct and try again.'); window.history.back();</script>";
                goto skip_insert;
              }
              $g_contact = $g_contact_digits;

              // Truncate fields to match DB column sizes to avoid Data too long errors
              $g_fname = mb_substr($g_fname, 0, 20);
              $g_lname = mb_substr($g_lname, 0, 20);
              $g_gender = mb_substr($g_gender, 0, 10);
              $g_email = mb_substr($g_email, 0, 30);

              // generate a random password for guest account
              $randpw = substr(md5(uniqid('', true)), 0, 8);

              // insert into patreg
              $pstmt = $con->prepare("INSERT INTO patreg(fname,lname,gender,email,contact,password,cpassword) VALUES(?, ?, ?, ?, ?, ?, ?)");
              if($pstmt){
                $pstmt->bind_param('sssssss', $g_fname, $g_lname, $g_gender, $g_email, $g_contact, $randpw, $randpw);
                $pexec = $pstmt->execute();
                if($pexec){
                  $new_pid = $con->insert_id;
                  // optionally set session for this guest
                  $_SESSION['pid'] = $new_pid;
                  $_SESSION['fname'] = $g_fname;
                  $_SESSION['lname'] = $g_lname;
                  $_SESSION['gender'] = $g_gender;
                  $_SESSION['email'] = $g_email;
                  $_SESSION['contact'] = $g_contact;
                  $pid_param = (int)$new_pid;
                } else {
                  echo "<script>alert('Unable to create guest patient. Please try again later.');</script>";
                  $pstmt->close();
                  goto skip_insert;
                }
                $pstmt->close();
              } else {
                echo "<script>alert('Database error: could not prepare patient insert');</script>";
                goto skip_insert;
              }
            } else {
              if(isset($_SESSION['username'])){
                echo "<script>alert('You are signed in as an admin. Please use the Admin Dashboard to create patient entries.'); window.location.href='admin-panel1.php';</script>";
                goto skip_insert;
              } else {
                echo "<script>alert('Unable to process your request: patient session missing. Please login and try again.'); window.location.href='index1.php';</script>";
                goto skip_insert;
              }
            }
          } else {
          ...
          // Insert appointment (only if $pid_param set)
          if(isset($pid_param)){
            $stmt = $con->prepare("INSERT INTO appointmenttb(pid,fname,lname,gender,email,contact,doctor,docFees,appdate,apptime,userStatus,doctorStatus) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '1', '1')");
            if($stmt){
              // ensure fname/lname/email/contact are taken from session (may have been just set for guest)
              $use_fname = isset($_SESSION['fname']) ? $_SESSION['fname'] : $fname;
              $use_lname = isset($_SESSION['lname']) ? $_SESSION['lname'] : $lname;
              $use_gender = isset($_SESSION['gender']) ? $_SESSION['gender'] : $gender;
              $use_email = isset($_SESSION['email']) ? $_SESSION['email'] : $email;
              $use_contact = isset($_SESSION['contact']) ? $_SESSION['contact'] : $contact;

              $stmt->bind_param('isssssssss', $pid_param, $use_fname, $use_lname, $use_gender, $use_email, $use_contact, $doctor, $docFees, $appdate, $apptime);
              $exec = $stmt->execute();
              if($exec){
...
if(isset($_GET["generate_bill"])){
  ...
  $content .= '
      <br/>
      <h2 align ="center"> City Hospital</h2></br>
...
<html lang="en">
  ...
  <a class="navbar-brand" href="#"><i class="fa fa-user-plus" aria-hidden="true"></i> City Hospital </a>
  ...
  <style >
    .bg-primary {
    background: -webkit-linear-gradient(left, #b22222, #ff6b6b);
  }
              ...
              <form class="form-group" method="post" action="admin-panel.php">
                <div class="row">
                  <?php if (empty($pid)) { ?>
                    ...
                    <div class="col-md-8"><input type="text" class="form-control" name="guest_fname" required></div><br><br>
                    <div class="col-md-4"><label>Last Name:</label></div>
                    <div class="col-md-8"><input type="text" class="form-control" name="guest_lname" required></div><br><br>
                    ...
                    <div class="col-md-8"><input type="email" class="form-control" name="guest_email" required></div><br><br>
                    <div class="col-md-4"><label>Contact Number:</label></div>
                    <div class="col-md-8"><input type="text" class="form-control" name="guest_contact" required></div><br><br>
                  <?php } ?>
