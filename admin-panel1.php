              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <?php 
                    $con=mysqli_connect("localhost","root","","myhmsdb");
                    global $con;
                    // show one record per unique email (keep the earliest pid) to hide duplicate entries
                    $query = "SELECT p.* FROM patreg p JOIN (SELECT MIN(pid) AS pid FROM patreg GROUP BY email) t ON p.pid = t.pid ORDER BY p.pid";
                    $result = mysqli_query($con,$query);
                    while ($row = mysqli_fetch_array($result)){
                      $pid = $row['pid'];
                      $fname = $row['fname'];
